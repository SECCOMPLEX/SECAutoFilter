...
API_ID = int(environ['API_ID', '13368804'])
API_HASH = environ['API_HASH', 'b6a4d6f4a6b93b0bd480b246188ae4ce']
BOT_TOKEN = environ['BOT_TOKEN', '5503733497:AAGHf196RyUYuRZWaW56kYCwfE3-fP7uT-A']
...
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '1573108290 1167310327 1903290765').split()]
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1001797222352').split()]
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '').split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else []
auth_channel = environ.get('AUTH_CHANNEL', '-1001575860047')
auth_grp = environ.get('AUTH_GROUP')
AUTH_CHANNEL = int(auth_channel) if auth_channel and id_pattern.search(auth_channel) else None
AUTH_GROUPS = [int(ch) for ch in auth_grp.split()] if auth_grp else None

# MongoDB information
DATABASE_URI = environ.get('DATABASE_URI', "mongodb+srv://Adiwatermarkbot:Adiwatermarkbot@cluster0.0je15.mongodb.net/myFirstDatabase?retryWrites=true&w=majority")
DATABASE_NAME = environ.get('DATABASE_NAME', "Adiwatermarkbot")
